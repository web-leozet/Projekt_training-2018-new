function Animation(id, fps, urls) {
    ...
    this.fps = 1000 / fps;  //200 ms                
    // скорость смены кадров в секунду
    this.imgElement = null;                 
    // элемент img в котором будет 
    //воспроизводиться анимация
    this.frames = new Array(urls.length);  
     // массив с загруженными изображениями.
    ...
    this.timer = null;                     
     // объект таймера, который будет создан через setInterval
    this.startOnLoad = true;
    // true если анимация должна запуститься сразу
    ...
    for (var i = 0; i < urls.length; i++) {
        ...
        this.frames[i].onload = countLoaded;
         ...
         // после загрузки изображения увеличить
         // счетчик loadedFrames
    ...
    function countLoaded() {
        ...
        if (current.loadedFrames == urls.length) {
            ...
            if (current.startOnLoad) 
              current.start(); // запустить анимацию если startOnLoad true
        }
    }

    // нижнее подчеркивание в имени метода 
    //указывает на то что он предназначен
    // только для внутреннего использования.
    // пользователь объекта Animation не 
    //должен запускать этот метод самостоятельно.
    this._nextFrame = function () {
        // при запуске метода через setInterval 
        //this будет ссылаться на window
        // поэтому вместо ссылки this
        // используется переменная current
        current.currentFrame = 
        (current.currentFrame + 1) % current.frames.length;
        
        current.imgElement.src = 
        current.frames[current.currentFrame].src;
    }
}
